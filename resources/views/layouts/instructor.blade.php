        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('instructor.dashboard') }}" 
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg transition {{ request()->routeIs('instructor.dashboard') ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100' }}">
                ...
                <a href="{{ route('instructor.programs.index') }}" 
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg transition {{ request()->routeIs('instructor.programs.*') ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                    </svg>
                    <span class="font-medium">Program Saya</span>
                </a>

                <!-- Absensi -->
                <a href="{{ route('instructor.attendance.index') }}" 
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg transition {{ request()->routeIs('instructor.attendance.*') ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('instructor.my-attendance.index') }}" 
                class="flex items-center space-x-3 px-4 py-3 rounded-lg transition {{ request()->routeIs('instructor.my-attendance.*') ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                    </svg>
                ...
                <a href="{{ route('instructor.profile.edit') }}" 
                   class="flex items-center space-x-3 px-4 py-3 rounded-lg transition {{ request()->routeIs('instructor.profile.*') ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <header class="bg-white shadow-sm border-b fixed top-0 right-0 left-0 lg:left-64 z-30">
                <div class="flex items-center justify-between h-16 px-6">
                    ...
                    <div class="flex-1 lg:flex-none">
                        <h2 class="text-xl font-semibold text-gray-800">@yield('title', 'Dashboard Instruktur')</h2>
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="relative" x-data="{ notificationOpen: false }">
                            ...
                            <div x-show="notificationOpen" @click.away="notificationOpen = false" 
                                 class="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border overflow-hidden z-50">
                                <div class="p-4 border-b">
                                    <h3 class="font-semibold">Notifikasi</h3>
                                </div>
                                <div class="max-h-96 overflow-y-auto">
                                    @forelse(Auth::user()->notifications->take(5) as $notification)
                                    <div class="p-4 hover:bg-gray-50 border-b {{ $notification->read_at ? '' : 'bg-blue-50' }}">
                                ...
                                <a href="#" class="block p-3 text-center text-sm text-blue-600 hover:bg-gray-50 border-t">Lihat semua</a>
                            </div>
                        </div>
                        
                        <!-- Profile Dropdown -->
                        <div class="hidden md:block relative" x-data="{ profileOpen: false }">
                            <button @click="profileOpen = !profileOpen" class="flex items-center space-x-3 pl-4 border-l">
                            ...
                            <div x-show="profileOpen" @click.away="profileOpen = false" 
                                 class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border overflow-hidden z-50">
                                <a href="{{ route('instructor.profile.edit') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
