...
@section('title', 'Tambah Unit Kompetensi')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Tambah Unit Kompetensi</h2>

        <form method="POST" action="{{ route('admin.independent-units.store') }}" class="space-y-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="code" class="block text-sm font-medium text-gray-700 mb-1">Kode Unit <span class="text-red-500">*</span></label>
                    <input type="text" name="code" id="code" value="{{ old('code') }}" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('code') border-red-500 @enderror">
                    @error('code')
            ...
            <div class="mt-6">
                ...
                <select name="skkni_id" id="skkni_id" class="w-full px-3 py-2 border rounded-lg">
                    ...
                    @foreach($skknis as $skkni)
                        <option value="{{ $skkni->id }}" {{ old('skkni_id') == $skkni->id ? 'selected' : '' }}>
            ...
            <div class="mt-6 border-t pt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                    <div>
                        <label for="nomor" class="block text-sm font-medium text-gray-700">Nomor SKKNI <span class="text-red-500">*</span></label>
                        <input type="text" name="nomor" id="nomor" value="{{ old('nomor') }}" required class="w-full px-3 py-2 border rounded-lg">
                    </div>
                    <div>
                        <label for="tanggal" class="block text-sm font-medium text-gray-700">Tanggal <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal') }}" required class="w-full px-3 py-2 border rounded-lg">
                    </div>
                    <div>
                        <label for="berlaku" class="block text-sm font-medium text-gray-700">Berlaku <span class="text-red-500">*</span></label>
                        <select name="berlaku" id="berlaku" required class="w-full px-3 py-2 border rounded-lg">
