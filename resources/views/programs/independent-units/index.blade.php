...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Kelola semua unit kompetensi secara independen</p>
        </div>
    </div>

    <!-- Form Tambah Unit (tanpa Program) -->
    <div class="bg-white rounded-lg shadow-sm border p-6" x-data="{ showForm: false }">
        ...
        <form x-show="showForm" x-collapse method="POST" action="{{ route('admin.independent-units.store') }}" class="space-y-4">
            @csrf
    ...
    <!-- Units List (hapus kolom Program) -->
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('admin.independent-units.show', $unit) }}" class="text-blue-600 hover:text-blue-800" title="Lihat">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('admin.independent-units.edit', $unit) }}" class="text-green-600 hover:text-green-800" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.independent-units.destroy', $unit) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus unit kompetensi ini?')">
                                    @csrf
                                    @method('DELETE')
