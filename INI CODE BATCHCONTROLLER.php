...
class BatchController extends Controller
{
    ...
    public function edit(Batch $batch)
    {
        ...
        $masterPrograms = MasterProgram::where('is_active', true)->get();
        $jenisPelatihans = JenisPelatihan::all();
        return view('batches.edit', compact('batch', 'masterPrograms', 'jenisPelatihans'));
    }
...
INI CODE BATCHES/INDEX.BLADE.PHP
@extends('layouts.app')
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <select name="jenis_pelatihan_id" id="editJenisPelatihanId" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                                <option value="">-- Pilih Jenis --</option>
                                                @foreach($jenisPelatihans as $jenis)
                                                    <option value="{{ $jenis->id }}" {{ old('jenis_pelatihan_id', $batch->jenis_pelatihan_id) == $jenis->id ? 'selected' : '' }}>
...
<div x-show="deleteModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title-delete" role="dialog" aria-modal="true" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
...
PERBAIKI CODE SUBMENU ITU DIKARENAKAN BELUM BISA TAMBAH DAN EDIT DATA KEMUDIAN TAMBAHKAN MODAL DELETE